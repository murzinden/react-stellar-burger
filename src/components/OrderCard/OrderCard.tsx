...
const OrderCard = ({order, ordersIngredientArray}: IProps) => {
    ...
    const commonPartCard = (
        <div className={cn(s.preview_container)}>
            <div className={cn(s.icons_container)}>
                ...
                {hiddenImg && <span className={cn(s.ingredient_icon)}>
                 ...
                 <div className={cn(s.img_last)}>
                   <p className={cn(s.number, "text text_type_digits-default")}>+{numberHiddenIngredients}</p>
                 </div>
               </span>}

            </div>
            <div className={cn(s.container_price)}>
            ...
            <Link
                ...
                <div>
                    ...
                    {order.status !== 'done' ? <p className="text text_type_main-default mt-2">{orderStatus()}</p> :
                        <p className={cn(s.done_color, "text text_type_main-default mt-2")}>{orderStatus()}</p>
                    }
                </div>
                {commonPartCard}
            </Link>
